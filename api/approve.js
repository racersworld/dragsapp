import { json, bad, db, require, audit } from "./_lib.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const r = (await db(`sign_ons?id=eq.${id}&select=id,event_id,result,signed_at`))[0];
    ...
    if (action === "refuse") {
      const out = await db(`sign_ons?id=eq.${id}`, { method: "PATCH", body: { result: "refused", approved_at: null, approved_by: u.id, refusal_reason: reason || "Refused at gate" } });
